...
| `treat-unstable-as-success`
| Boolean
| No
| Specifies whether a Jenkins `UNSTABLE` build result is treated as a success.
Default is `false`, which fails the step when the Jenkins build is unstable.
|===
